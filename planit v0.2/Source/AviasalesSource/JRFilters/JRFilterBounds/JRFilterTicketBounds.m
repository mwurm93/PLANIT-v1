...
- (BOOL)isReset {
    BOOL isReset = (self.filterPrice == self.maxPrice) &&  (self.filterGates.count == self.gates.count) &&
    (self.filterPaymentMethods.count == self.paymentMethods.count) && !self.filterMobileWebOnly;
    
    return isReset;
}

- (void)setFilterGates:(NSOrderedSet<JRSDKGate *> *)filterGates {
...
- (void)setFilterPaymentMethods:(NSOrderedSet<JRSDKPaymentMethod *> *)filterPaymentMethods {
